...
echo "=== FIDESlib GPU AddScalar with Maximum Slots ==="
...
rm -f gpu_scalaradd_16k_timing.csv
...
if [ -f gpu_scalaradd_16k_timing.csv ]; then
    ...
    if [ -n "$GPU_LINE" ]; then
        ...
        echo "✅ GPU scalar addition (16,384 slots): $GPU_TIME ns ($GPU_TIME_MS ms)"
        echo ""
...
echo ""
echo "Results saved to: gpu_scalaradd_16k_timing.csv"
echo ""
echo "💡 Note: For CPU comparison, use OpenFHE's EvalAdd(ct, scalar) directly"
