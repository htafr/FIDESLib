...
echo "=== FIDESlib GPU vs CPU MultScalar with Maximum Slots ==="
...
rm -f gpu_scalarmult_16k_timing.csv cpu_scalarmult_16k_timing.csv
...
if [ -f gpu_scalarmult_16k_timing.csv ]; then
    ...
    if [ -n "$GPU_LINE" ]; then
        ...
        echo "✅ GPU scalar multiplication (16,384 slots): $GPU_TIME ns"
    fi
fi

echo ""
echo "🔥 Running CPU scalar multiplication benchmark (16,384 slots)..."
echo "Note: Using OpenFHE CPU EvalMult(ct, scalar) for comparison"
timeout 300 ./fideslib-bench \
    --benchmark_filter="^GeneralFixture/MultScalarCPU/0/0/16384/11/manual_time$" \
    --benchmark_min_time=0.1s \
    --benchmark_out_format=csv \
    --benchmark_out=cpu_scalarmult_16k_timing.csv \
    --benchmark_counters_tabular=true 2>&1 | grep -v "The number of inputs is very large" || true

if [ -f cpu_scalarmult_16k_timing.csv ]; then
    CPU_LINE=$(grep "MultScalarCPU/0/0/16384/11/manual_time" cpu_scalarmult_16k_timing.csv | tail -1)
    if [ -n "$CPU_LINE" ]; then
        CPU_TIME=$(echo "$CPU_LINE" | cut -d',' -f3)
        echo "✅ CPU scalar multiplication (16,384 slots): $CPU_TIME ns"
        
        # Compare with GPU if both results exist
        if [ -n "$GPU_TIME" ] && [ -n "$CPU_TIME" ]; then
            echo ""
            echo "📊 SCALAR MULTIPLICATION PERFORMANCE COMPARISON (16,384 slots):"
...
echo ""
echo "Results saved to: gpu_scalarmult_16k_timing.csv, cpu_scalarmult_16k_timing.csv"
