...
echo "=== FIDESlib GPU AddPlaintext with Maximum Slots ==="
...
if [ -f gpu_ptadd_16k_timing.csv ]; then
    ...
    if [ -n "$GPU_LINE" ]; then
        ...
        echo "✅ GPU plaintext addition (16,384 slots): $GPU_TIME ns ($GPU_TIME_MS ms)"
        echo ""
...
echo ""
echo "Results saved to: gpu_ptadd_16k_timing.csv"
echo ""
echo "💡 Note: For CPU comparison, use OpenFHE's EvalAdd(ct, pt) directly"
